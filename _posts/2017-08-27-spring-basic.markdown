...
### Spring
   ...
   2003年，Spring 兴起之前，企业普遍使用 JAVA EE 技术开发企业级应用。

   - Java 本身的扩平台能力，可移植性强。

   - JavaEE 服务器内置了大量的容器服务，比如事务服务，JNDI服务，连接池服务等。

   - 可以基础 JavaEE API 进行开发并部署到应用中

   - 致命缺陷：

        - 研发困难，开发容易出问题，比如数据库连接泄露。

  Spring 的出现解决了上述问题。

  **什么是 Spring 呢？**

  为解决企业应用开发的复杂性而创建的轻量级控制反转 IOC 和面向切面 AOP 的容器框架。

  Spring 是开发者更专注于业务逻辑的开发，而不需要关注底层代码的具体。
